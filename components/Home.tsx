...
export const Home: React.FC = () => {
    ...
    return (
        <div className="animate-fade-in">
            <h1 className="text-4xl md:text-5xl font-extrabold mb-2 text-[#121e18] dark:text-white">{t("Welcome Back! 👋")}</h1>
            <p className="text-[#AFBD96] mb-8 text-lg">{t("Choose a deck to start learning. 💪")}</p>
                        ...
                        <div key={deck.id} className="bg-white dark:bg-[#344E41] rounded-xl shadow-lg border border-[#EDE9DE] dark:border-[#3A5A40] overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 relative">
                            ...
                            <div className="p-6">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <ModeButton onClick={() => handleStart(deck.id, GameMode.Matching)} icon={<PuzzlePieceIcon />} label={t("Matching")} />
                                    <ModeButton onClick={() => handleStart(deck.id, GameMode.Spelling)} icon={<PencilIcon />} label={t("Spelling")} />
                                </div>
                            </div>
                        </div>
                    ))}
